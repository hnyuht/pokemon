...
const cardBecomesActive = function (card) {
  ...
  if (length === 2) {
    ...
    if (
      activeCards[0].firstChild.dataset.dexid ===
      activeCards[1].firstChild.dataset.dexid
    ) {
      disableBoard();
      activeCards.forEach((c) => {
        c.style.backgroundImage =
          "radial-gradient(rgb(241, 241, 216), rgb(241, 245, 35))";
...
startButton.addEventListener("click", () => {
  if (isNaN(timerInput.value) || timerInput.value < 10) {
